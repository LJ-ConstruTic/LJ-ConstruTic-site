    ...
    return (
        <section className="w-full flex lg:flex-row flex-col lg:gap-0 h-auto">
            <div className="w-full flex relative h-auto">
                <Splide
                    className="w-full h-auto reset-padding-margin"
                    options={{
                        type: "loop",
                        arrows: false,
                        perPage: 1,
                    ...
                    {bannerArray?.map((item, idx) => (
                        <SplideSlide key={idx}>
                            <main data-aos="fade-down" className="w-full relative h-auto flex flex-col">
                                <section className="relative w-full h-auto max-w-[1280px] mx-auto">
                                    <div className="w-full top-12  absolute z-[100] flex flex-col items-center lg:items-start lg:gap-9 gap-3">
                                        <h4 className="lg:text-[52px] text-white text-3xl text-center md:text-start font-bold sm:leading-[50px] xl:leading-[65.35px]">
                                            {bannerData[0]?.tag[lang_current]}
                                        </h4>
                                        <p className="text-sm text-justify text-white lg:text-left xl:text-lg md:max-w-[700px]">
                                            {bannerData[1]?.tag[lang_current]}
                                        </p>
                                        <div className="flex justify-center md:justify-start">
                                            <a href="#contact" onClick={(e) => handleClick(e, "contact")}>
                                                <Button className="rounded-full px-5 bg-primary-green dark:text-white py-8 flex gap-1 items-center">
